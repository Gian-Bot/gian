...
let end;
if (a == b && b == c) {
end = `✨ *BIEN SERRANO! GANASTE ESTA WEBADA +${apuesta + apuesta} EXP*`
users.exp += apuesta
} else if (a == b || a == c || b == c) {
end = `🥱 *SIGUE INTENTANDO PAPI*\n*BONO DE +50 EXP*`
users.exp += 50
} else {
end = `😂 *HA PERDIDO, QUE BURRO!! ❌ -${apuesta} EXP*`
users.exp -= apuesta
}
//users.lastslot = new Date * 1
...
🎰 | *RANURAS* | 🎰 
 ...
 ────────
🎰 |   *SLOTS*   | 🎰`
await conn.sendHydrated(m.chat, `${s}\n\n${end}`, wm, null, md, '𝙂𝙄𝘼𝙉𝘽𝙊𝙏', null, null, [
...
if (command == 'slot2') {
let time = global.db.data.users[m.sender].lastslot + 30000
if (new Date - users.lastslot < 30000) throw `*𝙑𝙐𝙀𝙇𝙑𝘼 𝙀𝙉 ${msToTime(time - new Date())} 𝙋𝘼𝙍𝘼 𝘾𝙊𝙉𝙏𝙄𝙉𝙐𝘼𝙍 𝘼𝙋𝙊𝙎𝙏𝘼𝙉𝘿𝙊 𝘾𝙊𝙄𝙉𝙎* 🎰`
users.lastslot = new Date * 1
...
let end;
if (a == b && b == c) {
end = `✨ *SUELDO MÍNIMO! +${apuesta + apuesta} Coins*`
users.money += apuesta
} else if (a == b || a == c || b == c) {
end = `👹 *CASI!!, VUELVA A INTENTAR*\n*BONO DE +30 Coins*`
users.money += 30
} else {
end = `😂 *HA PERDIDO, QUE MONGOL! ❌ -${apuesta} Coins*`
users.money -= apuesta
}
//users.lastslot = new Date * 1
//return await m.reply(
...
let end;
if (a == b && b == c) {
end = `✨ *SUELDO MÍNIMO! +${apuesta + apuesta} Diamantes*`
users.limit += apuesta
} else if (a == b || a == c || b == c) {
end = `👹 *CASI!!, VUELVA A INTENTAR*\n*BONO DE +2 Diamantes*`
users.limit += 2
...
if (command == 'slot') {       
await conn.sendHydrated(m.chat, `*Elija en que apostará ${apuesta}*`, wm, null, md, '𝙂𝙄𝘼𝙉𝘽𝙊𝙏', null, null, [
