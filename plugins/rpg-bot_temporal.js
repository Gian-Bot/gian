...
let handler = async (m, { conn, args, usedPrefix, command, isPrems, isOwner, isROwner}) => {
  if (!global.db.data.settings[conn.user.jid].temporal) throw `${ag}𝙈𝙄 𝘾𝙍𝙀𝘼𝘿𝙊𝙍 𝙃𝘼 𝙍𝙀𝙎𝙏𝙍𝙄𝙉𝙂𝙄𝘿𝙊 𝙀𝙎𝙏𝘼 𝙁𝙐𝙉𝘾𝙄𝙊𝙉 𝘿𝙀 𝙄𝙉𝙂𝙍𝙀𝙎𝘼𝙍 𝙀𝙇 𝘽𝙊𝙏 𝙏𝙀𝙈𝙋𝙊𝙍𝘼𝙇𝙈𝙀𝙉𝙏𝙀 𝘼 𝙂𝙍𝙐𝙋𝙊𝙎 𝙑𝙐𝙀𝙇𝙑𝘼 𝙈Á𝙎 𝙏𝘼𝙍𝘿𝙀 𝙊 𝙎𝙄 𝙀𝙎 𝙊𝙒𝙉𝙀𝙍 𝘿𝙀𝙇 𝘽𝙊𝙏 𝙐𝙎𝙀 *#on temporal* 𝙋𝘼𝙍𝘼 𝘼𝘾𝙏𝙄𝙑𝘼𝙍`
...
conn.reply(m.chat, `${eg}👹 *SE HA UNIDO BOT AL GRUPO!!!*`, m).then(async() => { 
...
await conn.reply(m.chat, `${wm} *ES UN BOT DE WHATSAPP QUE TE AYUDARÁ A REALIZAR DIFERENTES ACTIVIDADES 🔥 AL PRIVADO O GRUPO Y TAMBIÉN TE VAS A DIVERTIR 🎈 CON SUS MULTIPLES FUNCIONES, DISFRUTA DEL BOT!!! 👹*\n\n🔥 *EL BOT FUE INVITADO POR:*\n${username}`, m)
