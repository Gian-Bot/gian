...
export async function handler(chatUpdate) {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `𝙉𝙀𝘾𝙀𝙎𝙄𝙏𝘼 𝙀𝙇 𝙉𝙄𝙑𝙀𝙇 ➡️ *${plugin.level}* 𝙋𝘼𝙍𝘼 𝙋𝙊𝘿𝙀𝙍 𝙐𝙎𝘼𝙍 𝙀𝙎𝙏𝙀 𝘾𝙊𝙈𝘼𝙉𝘿𝙊. 𝙏𝙐 𝙉𝙄𝙑𝙀𝙇 𝙀𝙎 ➡️ *${_user.level}* 𝘼𝘾𝙏𝙐𝘼𝙇𝙄𝙕𝘼 𝙏𝙐 𝙉𝙄𝙑𝙀𝙇 𝘾𝙊𝙉 𝙀𝙇 𝘾𝙊𝙈𝘼𝙉𝘿𝙊 *${usedPrefix}level*`, m)
	...
	await this.readMessages([m.key])
        ...
        if (!m.fromMem && m.text.match(/(Hola|como|cuando|donde|porque|hi|fine|GIAN|god|suerte|amor|odio|triste|gian|Gian|bot|bye|chao|saludo|siempre|veces|.-.|._.|:)|:(|:v|v:|o.o|;v|v;|v':|:'v)/gi)) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*𝙂𝙧𝙪𝙥𝙤 𝙂𝙚𝙣𝙞𝙖𝙡 👹*') :
                            ...
                            this.sendHydrated(id, text, groupMetadata.subject, apii.data, 'https://www.facebook.com/gianwalter28', '𝙂𝙄𝘼𝙉-𝘽𝙊𝙏', null, null, [
                            [(action == 'add' ? '𝙎𝙚 𝙪𝙣𝙞𝙤 🤑 | 𝙃𝙞!!' : '𝙎𝙚 𝙛𝙪𝙚 𝙪𝙣𝙤 | 𝘽𝙮𝙚'), '.s'],    
                            ['👹 𝙄𝙧 𝙖𝙡 𝙈𝙚𝙣𝙪', '/menu']
                            ], '', { mentions: [user]})
                           }
                }
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        owner: '╰⊱⚠️⊱ *𝘼𝘿𝙑𝙀𝙍𝙏𝙀𝙉𝘾𝙄𝘼 | 𝙒𝘼𝙍𝙉𝙄𝙉𝙂* ⊱⚠️⊱╮\n\n_*¡¡Este comando solo puede ser utilizado por Mí Creador!!*_',
